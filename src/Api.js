...
class Api {
	...
	*/
	...
	/**
	 * Get a unique identifier for a specific promise.
	 ...
	 *
	 * @return {string} Identifier for this promise
	 */
	getContentRevisionIdentifier() {
	...
	getJsonData() {
		return $.getJSON( this.getAjaxURL() )
			.then( result => {
					...
					if ( errCode === 'refresh' && this.retry <= this.maxRetries ) {
						...
						return new Promise( resolve => setTimeout( resolve, 1000 * this.retry ) )
							.then( () => {
								...
								window.console.log( 'WhoWroteThat Api::getData() retry ' + this.retry );
								this.retry++;
								// Recurse
				...
				// Report retry count.
				if ( this.retry > 1 ) {
					window.console.info( 'WhoWroteThat Api::getData() total retries: ' + ( this.retry - 1 ) );
				}
	 ...
	 */
	getData() {
		let promiseIdentifier = this.getContentRevisionIdentifier();
